...
4. Push openshift/release branch: template 3 CI files + update image mirroring from previous `upstream-v*` version (pattern: PR #74481; do not copy-paste verbatim). Do not open PRs — user opens them manually.

Always detect the current phase first. Do not skip the wait step after pushing the fork branch. Never run `gh pr create`.
